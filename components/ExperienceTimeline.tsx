...
/**
 * Roles and elected positions, shared by the home page's Experience section
 * and the standalone /experience page so the two can never drift apart.
 *
 * `headingLevel` exists because the same block appears under an <h1> on its
 * own page and under an <h2> on the home page; the role titles have to sit one
 * step below whichever it is.
 */
export default function ExperienceTimeline({
  ...
  const SubHeading = headingLevel === "h2" ? "h2" : "h3";

  return (
    <>
      {/* Timeline: a continuous rail with a node per role */}
      <ol className="relative flex flex-col gap-4 sm:pl-10">
        <span
          aria-hidden
        ...
        {experience.map((role, i) => (
          <li key={role.id} className="relative">
            <span
              aria-hidden
              className="absolute top-7 hidden h-2.5 w-2.5 rounded-full bg-primary ring-4 ring-plane sm:block"
              style={{ left: "-2.05rem" }}
            />
            <Reveal delay={i * 110} from="up">
              <div className="card box-hover p-6 sm:p-7">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-6">
                  <div>
                    <RoleHeading className="text-lg font-semibold tracking-tight">
                      {role.role}
                    </RoleHeading>
      ...
      <Reveal from="left">
        <SubHeading className="mt-16 text-2xl font-bold tracking-tight">
